    ...
    while (intentosRestantes > 0 && contieneGuiones(progreso)) {
      ...
      if (esIntentoLetraValida(entrada, letrasUsadas)) {
        ...
        if (palabraSecreta.indexOf(letra) == -1) {
          intentosRestantes--;
          System.out.println("Incorrecto. '" + letra + "' no está. Te quedan " + intentosRestantes + " intento"
        ...
        intentosRestantes--;
        System.out.println("Palabra incorrecta. Te quedan " + intentosRestantes + " intento"
            + (intentosRestantes == 1 ? "" : "s") + ".");
      }
    }
